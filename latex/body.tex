...
\begin{center}{\singlespacing\renewcommand{\arraystretch}{1.2}
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr\linewidth/2-2\tabcolsep\relax}>{\centering\arraybackslash}p{\dimexpr\linewidth/2-2\tabcolsep\relax}}
\textbf{TRƯỜNG ĐẠI HỌC XÂY DỰNG HÀ NỘI} \newline \textbf{KHOA CÔNG NGHỆ THÔNG TIN} & \textbf{CỘNG HOÀ XÃ HỘI CHỦ NGHĨA VIỆT NAM} \newline \textbf{Độc lập - Tự do - Hạnh phúc} \\
...
\textbullet{}  \textbf{Dữ liệu đầu vào:} dữ liệu vận hành tòa nhà mô phỏng (tòa nhà, tầng, căn hộ, cư dân, tài sản, vật tư); bộ dữ liệu hóa đơn tiếng Việt tổng hợp phục vụ huấn luyện – đánh giá OCR.

\textbullet{}  \textbf{Giả thiết:} (i) dữ liệu vận hành là dữ liệu giả lập, không gắn với một tòa nhà thật cụ thể; (ii) hệ thống triển khai ở mức nguyên mẫu (prototype) trên một máy chủ, chưa xét tải cực lớn và tính sẵn sàng cao (HA); (iii) người dùng truy cập qua trình duyệt hiện đại có kết nối Internet, thiết bị hỗ trợ camera cho chức năng nhận diện khuôn mặt; (iv) tài khoản và vai trò phân quyền được quản trị viên cấu hình sẵn; (v) tích hợp cổng thanh toán và thiết bị IoT nằm ngoài phạm vi, chỉ nêu định hướng.

\textbullet{}  \textbf{Tiêu chuẩn/quy chuẩn áp dụng:} kiến trúc dịch vụ và API theo phong cách \textbf{REST}, phản hồi thống nhất \texttt{ResponseDto<T>}; bảo mật theo \textbf{JWT} (RFC 7519), \textbf{OAuth 2.0/OIDC} (đăng nhập Google), \textbf{MFA TOTP} (RFC 6238) và \textbf{RBAC}, có tham chiếu khuyến nghị \textbf{OWASP}; phân tích – thiết kế theo \textbf{UML} (use case, tuần tự, trạng thái) và mô hình \textbf{ERD}, cơ sở dữ liệu chuẩn hóa quan hệ; giao diện \emph{responsive} đa thiết bị; mã nguồn tuân thủ quy ước .NET/TypeScript và quản lý phiên bản bằng Git; thuyết minh trình bày theo Quy định tổ chức thực hiện và đánh giá ĐATN của Trường ĐHXD Hà Nội (QĐ 483/QĐ-ĐHXDHN).

\textbullet{}  \textbf{Phần mềm/công cụ:} .NET 8, Next.js 16, PostgreSQL, Redis, Cloudinary; AI/OCR: PaddleOCR, VietOCR; draw.io, Graphviz, Git.
...
\end{tabular}}\end{center}
\begin{center}{\singlespacing\renewcommand{\arraystretch}{1.25}
\begin{tabular}{>{\centering\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep\relax}>{\centering\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep\relax}>{\centering\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep\relax}}
\textbf{Sinh viên} \newline \textit{(Ký, ghi rõ họ tên)} & \textbf{GV hướng dẫn} \newline \textit{(Ký, ghi rõ họ tên)} & \textbf{Chủ tịch Hội đồng} \newline \textit{(Ký, ghi rõ họ tên)} \\
\end{tabular}}\end{center}
\phantomsection
\chapter*{Lời nói đầu}
...
{\singlespacing\renewcommand{\arraystretch}{1.25}
\begin{longtable}{|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/4-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/4-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/4-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/4-2\tabcolsep-\arrayrulewidth\relax}|}
\hline
\textbf{Tiêu chí} & \textbf{Quản lý thủ công (Excel)} & \textbf{PM thương mại (Building Care / Landsoft / CyHome)} & \textbf{TownHub (đề xuất)} \\ \hline
\endfirsthead
\hline
\textbf{Tiêu chí} & \textbf{Quản lý thủ công (Excel)} & \textbf{PM thương mại (Building Care / Landsoft / CyHome)} & \textbf{TownHub (đề xuất)} \\ \hline
\endhead
\hline \multicolumn{4}{r}{\footnotesize\textit{(tiếp trang sau)}} \\
\endfoot
\endlastfoot
...
\phantomsection
\subsection*{1.3.3. Tham chiếu mô hình quy trình nghiệp vụ và điểm khác biệt của TownHub}
\addcontentsline{toc}{subsection}{1.3.3. Tham chiếu mô hình quy trình nghiệp vụ và điểm khác biệt của TownHub}
Các luồng nghiệp vụ lõi của phân hệ Tài sản không được thiết kế tùy tiện mà \emph{kế thừa những mô hình quy trình đã chuẩn hóa và kiểm chứng trong thực tế}, sau đó điều chỉnh cho bối cảnh quản lý vận hành tòa nhà tại Việt Nam. Việc bám theo các chuẩn mở, phổ biến giúp quy trình đúng thông lệ ngành, dễ bảo trì – mở rộng và tránh "phát minh lại bánh xe"; đồng thời làm rõ đóng góp riêng của đề tài nằm ở đâu. Nhóm chọn nguồn tham chiếu chính là bộ ERP mã nguồn mở \textbf{Odoo} (vì mở, tài liệu đầy đủ, được dùng rộng rãi và có thể đối chiếu trực tiếp mã nguồn) và khung \textbf{ITIL/ITSM} cho quản lý sự cố. Bốn luồng chính và lý do lựa chọn:
\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
  \item \textbf{Mua sắm (PR $\rightarrow$ PO $\rightarrow$ hóa đơn):} tham chiếu quy trình \emph{Procure-to-Pay} của Odoo Purchase — mô hình mua sắm nhiều cấp phổ biến nhất, tách bạch đề xuất – đơn mua – đối chiếu hóa đơn, phù hợp yêu cầu kiểm soát chi và phê duyệt của ban quản lý tòa nhà.
  \item \textbf{Bảo trì phòng ngừa (lịch bảo trì $\rightarrow$ lệnh công việc $\rightarrow$ checklist $\rightarrow$ nghiệm thu):} tham chiếu Odoo Maintenance (corrective \& preventive) — chuẩn tách bạch bảo trì phòng ngừa và khắc phục, gắn thiết bị với đội/kỹ thuật viên phụ trách.
  \item \textbf{Sự cố \& SLA (tiếp nhận $\rightarrow$ phân công $\rightarrow$ xử lý $\rightarrow$ giám sát SLA/leo thang):} tham chiếu \emph{ITIL Incident Management} và chính sách \emph{escalation} theo tài liệu ITSM của Atlassian — chuẩn de-facto cho vòng đời ticket và cam kết thời gian xử lý.
  \item \textbf{Kiểm kê kho (đếm tồn $\rightarrow$ đối chiếu $\rightarrow$ điều chỉnh):} tham chiếu \emph{Cycle Count / Inventory Adjustment} của Odoo Inventory — chuẩn kiểm kê định kỳ theo kho, đối chiếu tồn hệ thống với tồn thực đếm.
\end{itemize}

Bảng dưới đây tóm tắt: mỗi luồng kế thừa gì từ chuẩn tham chiếu và \emph{TownHub bổ sung/khác biệt ở đâu} so với nguồn tham khảo:

{\small\singlespacing\renewcommand{\arraystretch}{1.3}
\begin{longtable}{|>{\raggedright\arraybackslash}p{2.6cm}|>{\raggedright\arraybackslash}p{5.0cm}|>{\raggedright\arraybackslash}p{6.4cm}|}
\hline
\textbf{Luồng} & \textbf{Kế thừa từ chuẩn tham chiếu} & \textbf{Điểm khác biệt / bổ sung của TownHub} \\ \hline
\endfirsthead
\hline
\textbf{Luồng} & \textbf{Kế thừa từ chuẩn tham chiếu} & \textbf{Điểm khác biệt / bổ sung của TownHub} \\ \hline
\endhead
\hline \multicolumn{3}{r}{\footnotesize\textit{(tiếp trang sau)}} \\
\endfoot
\endlastfoot
Mua sắm (P2P) & Odoo Purchase: PR $\rightarrow$ duyệt $\rightarrow$ PO $\rightarrow$ hóa đơn; phê duyệt nhiều cấp; đối chiếu hóa đơn. & Thêm \textbf{OCR hóa đơn tiếng Việt} (VietOCR/PaddleOCR) và \textbf{đối chiếu ba chiều tự động} (nhà cung cấp qua MST / tổng tiền / danh mục vật tư) giữa hóa đơn AI bóc tách và PO; hóa đơn mua liên thông thẳng sang \textbf{ghi tăng tài sản} kèm bút toán Nợ/Có. \\ \hline
Bảo trì phòng ngừa (WO) & Odoo Maintenance: tách bảo trì phòng ngừa/khắc phục; lịch bảo trì; gán đội – KTV; lệnh công việc. & Gắn lệnh công việc với \textbf{checklist template + nghiệm thu}, \textbf{xuất vật tư trực tiếp từ kho} và \textbf{tự tạo PR khi thiếu vật tư}; liên thông với \textbf{khấu hao \& chi phí tài sản} trong cùng một hệ (không tách rời module). \\ \hline
Sự cố \& SLA & ITIL Incident Management (Atlassian): vòng đời ticket, phân loại ưu tiên, chính sách leo thang, cam kết SLA. & Áp cho \textbf{vận hành tòa nhà} (không phải IT): cấu hình SLA riêng, \textbf{nhật ký leo thang} tự động, \textbf{ticket $\rightarrow$ PR} khi cần vật tư, chụp ảnh trước/sau và \textbf{đánh giá của cư dân}; liên thông trực tiếp với kho, mua sắm và tài sản. \\ \hline
Kiểm kê kho & Odoo Inventory: cycle count theo kho; đối chiếu tồn hệ thống với tồn thực đếm; điều chỉnh tồn. & Bổ sung \textbf{phiếu kiểm kê có mã kỳ}, tính \textbf{chênh lệch số lượng \& giá trị}, \textbf{lưu lịch sử kiểm kê} và \textbf{tự sinh giao dịch điều chỉnh tồn}; gắn vật tư kiểm kê với luồng bảo trì – sự cố của cùng phân hệ. \\ \hline
\end{longtable}}
\noindent\textit{\small Nguồn tham chiếu (truy cập 07/2026): Mua sắm — Odoo Purchase, \url{https://www.odoo.com/documentation/18.0/applications/inventory_and_mrp/purchase.html}; Bảo trì — Odoo Maintenance, \url{https://www.odoo.com/documentation/18.0/applications/inventory_and_mrp/maintenance/maintenance_requests.html}; Sự cố \& SLA — ITIL/ITSM Incident Management, \url{https://www.atlassian.com/incident-management} và \url{https://www.atlassian.com/incident-management/on-call/escalation-policies}; Kiểm kê — Odoo Inventory (Cycle counts), \url{https://www.odoo.com/documentation/18.0/applications/inventory_and_mrp/inventory/warehouses_storage/inventory_management/cycle_counts.html}.}

\noindent Tóm lại, TownHub \emph{không sao chép} các quy trình tham chiếu mà kế thừa bộ khung đã chuẩn hóa rồi \emph{tích hợp liên thông bốn luồng trong một phân hệ} và bổ sung các thành phần đặc thù (OCR hóa đơn tiếng Việt, đối chiếu ba chiều, bút toán kế toán tài sản, SLA cho vận hành tòa nhà, lịch sử kiểm kê) — đây là phần đóng góp làm nên khác biệt so với việc dùng trực tiếp các sản phẩm sẵn có.
...
Cơ sở dữ liệu lõi của Base tập trung ở schema auth, quản lý người dùng, phân quyền theo vai trò (RBAC), phiên đăng nhập, xác thực đa yếu tố (MFA/TOTP), xác minh email – đặt lại mật khẩu và nhật ký hoạt động. Sơ đồ thực thể – liên kết (ERD) được trình bày ở Hình 3.3.

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.9\textheight,keepaspectratio]{images/erd_auth.pdf}
...
mfa\_secret (AuthMfaSecret) & Bí mật MFA/TOTP: type, status, secret, mã khôi phục & Thuộc một người dùng \\ \hline
email\_verification (AuthEmailVerification) & Mã xác minh email: codeHash, hạn dùng, thời điểm dùng & Thuộc một người dùng \\ \hline
password\_reset (AuthPasswordReset) & Mã đặt lại mật khẩu: codeHash, purpose, hạn dùng & Thuộc một người dùng \\ \hline
\end{tabular}}\end{center}
\phantomsection
...
\figland{images/img09.pdf}{Hình 3.4. Biểu đồ tuần tự: Đăng nhập có MFA và cấp JWT}{Hình 3.4. Biểu đồ tuần tự: Đăng nhập có MFA và cấp JWT}
...
\end{figure}
Về phân quyền, hệ thống dùng RBAC: mỗi \textbf{quyền} gồm tên policy dạng PascalCase (ví dụ \texttt{AssetCreate}) và mã \texttt{resource.action} (\texttt{asset.create}); quyền được gán cho \textbf{vai trò}, rồi vai trò gán cho \textbf{người dùng}. Toàn bộ quyền khai báo tập trung tại \texttt{PermissionConstants.cs} (nguồn sự thật duy nhất); mỗi quyền tự sinh một Authorization Policy và được gắn lên API bằng \texttt{[Authorize(Policy="...")]}. Nhờ đó phân quyền thông suốt đầu–cuối: seed CSDL $\rightarrow$ gán vai trò $\rightarrow$ nhúng claim vào JWT $\rightarrow$ frontend ẩn/hiện chức năng $\rightarrow$ backend chặn ở tầng API. Hệ thống có 74 quyền, chia hai nhóm CORE (nền tảng, 43 quyền) và MODULE (nghiệp vụ, 31 quyền).

Khi kiểm thử theo vai trò, nhóm thực hiện ba điều chỉnh để bảo đảm nguyên tắc \textbf{đặc quyền tối thiểu} (least privilege):
\begin{itemize}[leftmargin=1.4em,itemsep=3pt,topsep=2pt]
  \item \textbf{Tách quyền quá rộng.} Vài quyền gộp nhầm nhiều chức năng: \texttt{asset.view} (tra cứu tài sản) lỡ mở cả khối sổ sách kế toán tài sản, còn \texttt{inventory.transaction} (xuất/nhập kho) lại cho sửa danh mục vật tư và danh sách kho. Đã tách thành các quyền mịn hơn — \texttt{asset.accounting}, \texttt{inventory.manage}, \texttt{resident.access\_review}, \texttt{resident.face\_register} — chỉ gán cho vai trò phụ trách, còn Kỹ thuật viên giữ đúng phạm vi thi hành hiện trường.
  \item \textbf{Policy tổng hợp cho endpoint dùng chung.} Một endpoint cập nhật phiếu (\texttt{work-order/update}, \texttt{ticket/update}) phục vụ cả người quản lý (sửa nội dung) lẫn kỹ thuật viên (đẩy trạng thái khi check-in). Thay vì tách thêm endpoint, hệ thống dùng \textit{policy theo phép hoặc}: \texttt{WorkOrderWrite} = \texttt{create} $\vee$ \texttt{execute} và \texttt{TicketWrite} = \texttt{create} $\vee$ \texttt{resolve} (cài bằng \texttt{RequireAssertion}, không lưu trong CSDL). Tương tự, kỹ thuật viên chỉ được \texttt{procurement.request} để lập đề xuất mua vật tư và xem lại phiếu của mình qua \texttt{ProcurementRead} = \texttt{view} $\vee$ \texttt{request} (lọc theo \texttt{requestedByUserId}), không thấy toàn bộ hồ sơ mua sắm; quyền duyệt vẫn thuộc cấp trên nên tách bạch người đề xuất và người duyệt.
  \item \textbf{Không siết quá tay.} Đặc quyền tối thiểu phải xác định theo \textit{luồng nghiệp vụ thực tế}, không theo tên phân hệ: Kế toán cần \texttt{inventory.view} để đối chiếu từng dòng hóa đơn với danh mục vật tư khi xác nhận hóa đơn (thiếu quyền này thì danh sách vật tư rỗng, mọi dòng đều "chưa khớp" và không lưu được), nhưng vẫn không có \texttt{inventory.transaction}, \texttt{inventory.manage} hay \texttt{inventory.audit}.
\end{itemize}

Ngoài chặn ở tầng API, hệ thống còn giới hạn \textbf{tầm nhìn dữ liệu theo bản ghi}: kỹ thuật viên chỉ xem phiếu bảo trì và phiếu sự cố được phân công cho mình. Việc phân biệt dựa trên \emph{mã quyền} chứ không theo tên vai trò (vốn dễ đổi): người chỉ có quyền thi hành (\texttt{workorder.execute} / \texttt{ticket.resolve}) mà thiếu quyền điều phối (\texttt{assign}, \texttt{close}, \texttt{create}) bị giới hạn theo phân công, còn kỹ sư trưởng có cả hai nhóm quyền thì thấy toàn bộ. Bộ lọc cài bằng điều kiện \texttt{assignedToUserId} trong truy vấn danh sách; đồng thời mọi thao tác theo mã phiếu đều qua một hàm kiểm tra chung, trả về HTTP 403 nếu phiếu không thuộc người gọi (chống dò định danh). Ở frontend, ngoài ẩn menu theo quyền, mỗi trang được bọc bởi thành phần \texttt{RequirePermission} để chặn truy cập trực tiếp bằng URL; riêng trang bật camera giám sát, phần kiểm tra quyền đặt bao ngoài để thiết bị không khởi động trước khi quyền được xác nhận.

Các nhóm API chính của phân hệ Xác thực và Tài khoản được tổng hợp ở Bảng 3.1.

\begin{center}{\small\singlespacing\renewcommand{\arraystretch}{1.2}
...
\hline
\rowcolor{gray!15}\textbf{Nhóm} & \textbf{Endpoint tiêu biểu} & \textbf{Chức năng} \\ \hline
Đăng nhập & POST /login/StaffLogin, /login/userLogin & Đăng nhập nhân viên / cư dân \\ \hline
MFA \& token & POST /login/mfa/verify; POST /login/auth/refresh & Xác thực 2 lớp; làm mới token (rotation) \\ \hline
Đăng xuất & POST /login/logout, /login/logout/all & Đăng xuất một / mọi thiết bị \\ \hline
Đăng ký & POST /register; /register/create-bql; /register/create-resident & Cư dân tự đăng ký; Admin tạo BQL; BQL tạo cư dân \\ \hline
Tài khoản & GET /account/mfa/status; POST /account/mfa/totp/start$|$confirm$|$disable & Bật/tắt xác thực TOTP \\ \hline
Mật khẩu & POST /account/password/change/*, /account/password/forgot/* & Đổi \& quên mật khẩu (qua email hoặc MFA) \\ \hline
Phiên & GET /userSession/getall, /userSession/getByUserId/\{id\} & Quản lý phiên đăng nhập đa thiết bị \\ \hline
\end{tabular}}\end{center}
\textit{Bảng 3.1. Các nhóm API phân hệ Xác thực \& Tài khoản.}

Trên cơ sở danh mục quyền nêu trên, hệ thống seed sẵn sáu vai trò nghiệp vụ bám theo tác nhân trong biểu đồ use case. Ma trận phân quyền rút gọn theo các nhóm chức năng chính được trình bày ở Bảng 3.2; ký hiệu \checkmark{} là được cấp, dấu -- là không được cấp.

\begin{center}\begin{minipage}{\linewidth}\centering
{\footnotesize\singlespacing\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.31\linewidth}|c|c|c|c|c|}
\hline
\rowcolor{gray!15}\textbf{Nhóm chức năng} & \textbf{QTV} & \textbf{BQL} & \textbf{KS trưởng} & \textbf{KTV} & \textbf{Kế toán} \\ \hline
Tra cứu tài sản (\texttt{asset.view}) & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\ \hline
Khai báo / sửa / xoá tài sản & \checkmark & sửa & \checkmark & -- & sửa \\ \hline
Sổ sách kế toán TSCĐ (\texttt{asset.accounting}) & \checkmark & \checkmark & -- & -- & \checkmark \\ \hline
Lập lịch \& tạo Work Order & \checkmark & \checkmark & \checkmark & -- & -- \\ \hline
Phân công kỹ thuật viên & \checkmark & \checkmark & \checkmark & -- & -- \\ \hline
Check-in \& thực hiện bảo trì & \checkmark & -- & \checkmark & \checkmark & -- \\ \hline
Nghiệm thu \& đóng phiếu & \checkmark & \checkmark & \checkmark & -- & -- \\ \hline
Tiếp nhận \& xử lý sự cố & \checkmark & phân công & \checkmark & xử lý & -- \\ \hline
Phạm vi phiếu WO/sự cố được xem & toàn bộ & toàn bộ & toàn bộ & \textit{được giao} & -- \\ \hline
Tra cứu tồn kho \& vật tư (\texttt{inventory.view}) & \checkmark & -- & \checkmark & \checkmark & \checkmark \\ \hline
Xuất / nhập kho (\texttt{inventory.transaction}) & \checkmark & -- & \checkmark & \checkmark & -- \\ \hline
Danh mục vật tư \& kho (\texttt{inventory.manage}) & \checkmark & -- & \checkmark & -- & -- \\ \hline
Kiểm kê kho (\texttt{inventory.audit}) & \checkmark & -- & \checkmark & -- & -- \\ \hline
Mua sắm: đề xuất / duyệt / đặt hàng & \checkmark & \checkmark & đề xuất & \textit{đề xuất} & đặt, hoá đơn \\ \hline
Camera \& người lạ ra/vào (\texttt{resident.access\_review}) & \checkmark & \checkmark & -- & -- & -- \\ \hline
Quản trị tài khoản \& phân quyền & \checkmark & một phần & -- & -- & -- \\ \hline
\end{tabular}}

\vspace{0.6em}
{\raggedright\textit{Bảng 3.2. Ma trận phân quyền rút gọn theo vai trò nghiệp vụ (QTV: quản trị viên; BQL: ban quản lý; KTV: kỹ thuật viên).}\par}
...
Phân hệ quản lý người dùng cung cấp các thao tác CRUD tài khoản (UserController) kèm trạng thái hoạt động, vai trò và thông tin hồ sơ. Base cung cấp định danh trung tâm để các phân hệ khác liên kết (ví dụ phân hệ Quản lý dân cư gắn tài khoản với cư dân/căn hộ). Việc quản trị vai trò và quyền được tách thành các controller riêng: \texttt{RoleController} (tạo/sửa/nhân bản vai trò), \texttt{PermissionController} (danh mục quyền, tra quyền theo vai trò/người dùng), \texttt{UserRoleController} (gán vai trò cho người dùng). Mỗi API quản trị đều có biến thể thường và biến thể \texttt{admin/} với chính sách quyền cao hơn, tránh việc một quản trị viên cấp thấp tự nâng quyền cho mình. Các nhóm API được tổng hợp ở Bảng 3.3.

\begin{center}{\small\singlespacing\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.18\linewidth}|>{\raggedright\arraybackslash}p{0.46\linewidth}|>{\raggedright\arraybackslash}p{0.28\linewidth}|}
\hline
\rowcolor{gray!15}\textbf{Nhóm} & \textbf{Endpoint tiêu biểu} & \textbf{Chức năng} \\ \hline
Người dùng & GET /user/getAllUsers, /user/me, /user/by-role; PUT /user/update/profile; DELETE /user/deleteUser & Danh sách, hồ sơ, lọc theo vai trò, cập nhật, xoá \\ \hline
Vai trò & GET /roles/getall; POST /roles/addRole, /roles/clonerole; PUT /roles/updateRole; DELETE /roles/deleteRole/\{id\} & CRUD \& nhân bản vai trò \\ \hline
Quyền & GET /permissions/getall, /permissions/getbyRoleID/\{id\}, /permissions/getbyUserID/\{id\} & Danh mục quyền; tra quyền theo vai trò/người dùng \\ \hline
Gán vai trò & POST /user-roles/assign-roles, /user-roles/admin/assign-roles & Đồng bộ vai trò cho người dùng \\ \hline
Nhật ký & GET /auditLog/getbyuser/\{id\} & Truy vết hành động theo người dùng \\ \hline
\end{tabular}}\end{center}
\textit{Bảng 3.3. Các nhóm API phân hệ Quản lý người dùng \& Phân quyền.}

Giao diện quản trị tài khoản người dùng và màn hình vai trò – quyền (RBAC) được thể hiện ở Hình 3.7 và Hình 3.8.

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{images/real_users.png}
\caption*{\small Hình 3.7. Giao diện Quản trị tài khoản người dùng}
\addcontentsline{lof}{figure}{Hình 3.7. Giao diện Quản trị tài khoản người dùng}
\end{figure}
\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{images/real_permissions.png}
\caption*{\small Hình 3.8. Giao diện Danh mục quyền \& phân quyền theo vai trò (RBAC)}
\addcontentsline{lof}{figure}{Hình 3.8. Giao diện Danh mục quyền \& phân quyền theo vai trò (RBAC)}
\end{figure}

\phantomsection
\subsection*{3.4.3. Notification Engine - Thông báo}
\addcontentsline{toc}{subsection}{3.4.3. Notification Engine - Thông báo}
Phân hệ thông báo cho phép gửi thông báo đa kênh (push, email, SMS) qua NotificationController, quản lý mẫu thông báo tái sử dụng qua NotificationTemplateController và tra cứu lịch sử thông báo đã gửi. Mỗi thông báo lưu đối tượng nhận (audience), kênh gửi, số lượng gửi/thành công/thất bại và trạng thái (đã gửi, đã đọc), cho phép theo dõi tỉ lệ tiếp cận. Đây là dịch vụ dùng chung cho các phân hệ nghiệp vụ, ví dụ cảnh báo tồn kho thấp, nhắc hạn bảo trì hoặc cảnh báo sắp/đã quá hạn SLA — các phân hệ chỉ cần gọi dịch vụ thông báo với mã tham chiếu (referenceType, referenceId) mà không phải tự xây kênh gửi riêng. Giao diện soạn và theo dõi thông báo được thể hiện ở Hình 3.9.

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{images/real_notifications.png}
\caption*{\small Hình 3.9. Giao diện Quản lý \& lịch sử thông báo}
\addcontentsline{lof}{figure}{Hình 3.9. Giao diện Quản lý \& lịch sử thông báo}
\end{figure}
...
Phân hệ cấu hình quản lý các tham số chung của hệ thống và các danh mục dùng chung như loại tài sản, loại phí, loại dịch vụ. Mỗi tham số lưu dạng khóa–giá trị kèm kiểu dữ liệu và cờ công khai (\texttt{SystemConfig}: key, value, dataType, isPublic), cho phép thay đổi hành vi hệ thống mà không cần sửa mã. Một số nhóm cấu hình tiêu biểu: định dạng \& độ dài sinh mã (mã tài sản, phiếu, hóa đơn), phương pháp khấu hao mặc định, ngưỡng cảnh báo tồn kho, thời gian sống của token, và thông tin hiển thị khu (tên, logo). Việc tập trung danh mục giúp chuẩn hóa dữ liệu đầu vào và cho phép các phân hệ tự động áp dụng giá trị mặc định.

\phantomsection
\subsection*{3.4.5. Reporting - Dashboard và KPI}
\addcontentsline{toc}{subsection}{3.4.5. Reporting - Dashboard và KPI}
Phân hệ báo cáo tổng hợp số liệu toàn hệ thống thành dashboard trực quan và các báo cáo KPI, báo cáo chi phí (route /reports, /reports/kpi, /reports/cost; KpiSnapshotController, CostTrackingController). Bảng điều khiển hiển thị các chỉ số nhanh (tổng tài sản, sự cố đang mở, work order tới hạn, chi phí bảo trì trong tháng), biểu đồ dòng tiền theo tháng và thống kê sự cố theo loại. Báo cáo KPI theo dõi hiệu suất xử lý (tỉ lệ đúng hạn SLA, thời gian xử lý trung bình), còn báo cáo chi phí bóc tách chi phí bảo trì – vật tư – mua sắm theo kỳ. Số liệu được thu thập từ các phân hệ nghiệp vụ (tài sản, kho, sự cố) qua tham chiếu Guid cross-service, phục vụ ra quyết định vận hành. Giao diện Bảng điều khiển được thể hiện ở Hình 3.6.
...
\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
  ...
  \item Các phân hệ tham chiếu định danh cross-service bằng Guid (ví dụ buildingId, floorId của Base) và KHÔNG dùng navigation property xuyên module, giữ cho ba module độc lập, dễ bảo trì và triển khai. Riêng tham chiếu tới \textit{người dùng} là ngoại lệ: khóa chính của tài khoản trong module Auth là số nguyên chứ không phải Guid, nên các cột trỏ về người dùng được thiết kế thành cặp \texttt{<vaiTrò>UserId} (số nguyên, để tra ngược về tài khoản) kèm \texttt{<vaiTrò>Name} (tên hiển thị, để không phải gọi chéo module chỉ để hiện tên) – áp dụng thống nhất cho kỹ thuật viên được phân công của Work Order và phiếu sự cố, người đề xuất mua hàng và người xác nhận thanh toán hóa đơn. Cách này thay cho việc lưu tên do người dùng gõ tay vào trường ghi chú: tên gõ tay không truy vết được về tài khoản nào đã thực hiện thao tác, dễ sai chính tả và không dùng được để thống kê hay đối soát trách nhiệm.
...
Kỹ sư trưởng & Lập lịch bảo trì, phân công, nghiệm thu; quản lý danh mục vật tư \& kho. \\ \hline
Kỹ thuật viên & Quét QR check-in, thực hiện Work Order \& checklist, xử lý sự cố, lĩnh vật tư. Chỉ tra cứu (không sửa) danh mục tài sản, vị trí, lịch bảo trì, danh mục vật tư, kho; không truy cập khối sổ sách kế toán. \\ \hline
\end{tabular}}\end{center}
...
\figland{images/img12.pdf}{Hình 4.1. Biểu đồ Use Case – Quản lý tài sản \& bảo trì}{Hình 4.1. Biểu đồ Use Case – Quản lý tài sản \& bảo trì}
\begin{center}{\singlespacing\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep-\arrayrulewidth\relax}|}
\hline
...
\figland{images/img13.pdf}{Hình 4.2. Biểu đồ tuần tự: Ghi tăng tài sản}{Hình 4.2. Biểu đồ tuần tự: Ghi tăng tài sản}
...
/api/​asset/​asset-depreciation/* & Tra cứu \& chạy khấu hao kỳ & AssetAccounting \\ \hline
/api/​asset/​asset-document/*, /asset-disposal/* & Chứng từ kế toán \& thanh lý & AssetAccounting \\ \hline
/api/​asset/​asset-report/* & Nhật ký chung, sổ cái, báo cáo & AssetAccounting \\ \hline
/api/​asset/​maintenance-schedule/* & Lịch bảo trì (get-overdue) & WorkOrderView/​Create \\ \hline
/api/​asset/​work-order/get-*, /assign-technician & Tra cứu \& phân công Work Order & WorkOrderView/​Assign \\ \hline
/api/​asset/​work-order/update & Cập nhật phiếu; KTV check-in & WorkOrderWrite (create $\vee$ execute) \\ \hline
/api/​asset/​work-order/add-checklist-response & Ghi kết quả checklist & WorkOrderExecute \\ \hline
/api/​asset/​ticket/update & Cập nhật phiếu; KTV ghi kết quả & TicketWrite (create $\vee$ resolve) \\ \hline
/api/​asset/​warehouse/*, /material/* (ghi) & Danh mục vật tư \& kho & InventoryManage \\ \hline
/api/​asset/​inventory-transaction/create & Xuất / nhập kho & InventoryTransaction \\ \hline
/api/​asset/​purchase-request/create, /submit & KTV đề xuất mua vật tư & ProcurementRequest \\ \hline
/api/​asset/​purchase-request/get-* & Xem phiếu đề xuất (KTV: của mình) & ProcurementRead \\ \hline
/api/​asset/​purchase-request/approve$|$reject & Duyệt / từ chối đề xuất & ProcurementApprove \\ \hline
\end{tabular}}\end{center}
Hai policy \texttt{WorkOrderWrite} và \texttt{TicketWrite} là policy tổng hợp theo phép hoặc (xem mục 3.4.1), cho phép kỹ thuật viên đẩy trạng thái phiếu khi check-in và khi ghi nhận kết quả khắc phục mà không cần cấp thêm quyền tạo/xoá phiếu.
\phantomsection
...
Quy trình kiểm kê kho định kỳ - chốt số liệu tồn hệ thống, đếm thực tế tại kho, đối chiếu chênh lệch và phê duyệt phiếu điều chỉnh - được minh họa ở Hình 4.9. Mỗi kỳ kiểm kê được lưu thành một phiếu \textbf{StockTake} (gồm phần đầu phiếu và các dòng chi tiết \textbf{StockTakeLine}), ghi lại kho, kỳ, người thực hiện (chọn từ danh sách nhân sự, không nhập tay) cùng số tồn sổ sách - thực đếm - chênh lệch của từng vật tư. Toàn bộ các kỳ kiểm kê đều được lưu để tra cứu \emph{lịch sử kiểm kê} - kể cả kỳ khớp sổ hoàn toàn (không có chênh lệch); với các dòng có chênh lệch, hệ thống tự sinh giao dịch điều chỉnh (ADJUST) tham chiếu về mã phiếu kiểm kê và cập nhật lại mức tồn.
...
\medskip
\noindent\textbf{Đối chiếu ba chiều tại bước xác nhận. }Kết quả OCR không được ghi thẳng thành hóa đơn mà phải qua màn hình \textit{Đối chiếu \& xác nhận}, nơi hệ thống so sánh chứng từ AI bóc tách với đơn mua (PO) đã liên kết trên ba tiêu chí độc lập:

\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
  \item \textbf{Nhà cung cấp: }mã số thuế người bán trên hóa đơn được dùng để tra nhà cung cấp; nếu không khớp mã số thuế thì đối sánh tiếp theo tên (đã chuẩn hóa bỏ dấu). Kết quả phải trùng với nhà cung cấp của PO.
  \item \textbf{Tổng tiền: }tổng cộng của hóa đơn (tiền hàng cộng thuế) so với tổng tiền PO, chấp nhận sai số dưới một đơn vị tiền.
  \item \textbf{Danh mục vật tư: }tập vật tư mà PO yêu cầu so với tập vật tư của các dòng hóa đơn đã ánh xạ được. Hệ thống chỉ ra đích danh hai loại sai lệch – vật tư \textit{PO yêu cầu nhưng hóa đơn không có} (giao thiếu hoặc OCR đọc sót) và vật tư \textit{hóa đơn có nhưng PO không yêu cầu} (giao sai hoặc tính thêm hàng).
\end{itemize}

Tiêu chí thứ ba là cần thiết vì hai tiêu chí đầu có thể cùng đạt trong khi hàng giao vẫn sai: một hóa đơn đúng nhà cung cấp và bằng đúng tổng tiền PO vẫn có thể chứa mặt hàng khác với mặt hàng đã đặt. Mỗi dòng vật tư của PO được hiển thị kèm trạng thái \textit{có trên hóa đơn} hoặc \textit{thiếu} để kế toán đối chiếu trực quan. Khi PO chưa khai báo dòng vật tư, tiêu chí này được đánh dấu là chưa có cơ sở đối chiếu thay vì bị coi là sai lệch. Kế toán vẫn được phép xác nhận hóa đơn lệch chuẩn, nhưng hệ thống bắt buộc xác nhận lại và liệt kê rõ từng điểm lệch – bảo đảm sai lệch là một quyết định có chủ đích chứ không phải do bỏ sót.
...
Mục tiêu & Số hóa hóa đơn giấy (ảnh chụp hoặc scan) thành các trường có cấu trúc (số hóa đơn, ngày, MST, tiền hàng, thuế, tổng) và bảng chi tiết hàng hóa, giảm nhập liệu thủ công và sai sót. \\ \hline
...
Kích hoạt & Người dùng chọn chức năng \emph{Upload hóa đơn} và tải lên tệp ảnh (JPG/PNG). \\ \hline
Luồng chính &
1. Người dùng chọn engine OCR (một trong ba: \texttt{paddleocr} mặc định, \texttt{vietocr}, \texttt{paddledet\_viet}) và tải tệp hóa đơn lên.\newline
...
UC-INV-03 & Lưu \& tra cứu lịch sử kiểm kê kho & Kỹ sư trưởng \\ \hline
...
StockTake / StockTakeLine & Phiếu kiểm kê (đầu phiếu \& dòng chi tiết đối chiếu tồn), lưu lịch sử mọi kỳ kiểm kê \\ \hline
...
/api/​asset/​stock-take/* & Kiểm kê: tạo phiếu (create), lịch sử (get-all), chi tiết (get) \\ \hline
...
/inventory/​stock-taking, /inventory/​stock-taking/​history, /inventory/​transactions/​new & Kiểm kê, lịch sử kiểm kê \& tạo giao dịch \\ \hline
...
Dịch vụ OCR được viết bằng Python/FastAPI, tách riêng khỏi backend .NET và gọi qua HTTP (POST /extract). Việc nhận dạng chạy bất đồng bộ: mỗi yêu cầu được đưa vào hàng đợi (OcrJobQueue) và một tiến trình nền (OcrProcessingWorker) lần lượt lấy ra xử lý, nhờ đó giao diện không phải chờ. Ảnh đầu vào là tệp ảnh JPG/PNG (ảnh chụp hoặc scan hóa đơn). Dịch vụ cài đặt \textbf{ba engine OCR tự chủ} và người dùng chọn trực tiếp trên giao diện (lưu ở cột \texttt{ocr\_jobs.ocrEngine}): \texttt{paddleocr} (mặc định), \texttt{vietocr} (EasyOCR phát hiện + VietOCR nhận dạng) và biến thể lai \texttt{paddledet\_viet} (DBNet của Paddle phát hiện + VietOCR nhận dạng) — vừa để so sánh mô hình vừa chọn engine phù hợp cho từng loại hóa đơn. Dù dùng engine nào, kết quả cũng được quy về một cấu trúc dòng chuẩn hóa [\{text, box, prob\}] rồi đưa vào chung một hàm bóc tách trường (dựa trên biểu thức chính quy và vị trí ô chữ), nên việc đổi engine không ảnh hưởng tới khâu lấy trường.
...
\caption*{\small Hình 4.16. Kiến trúc pipeline trích xuất hóa đơn của dịch vụ OCR}
\addcontentsline{lof}{figure}{Hình 4.16. Kiến trúc pipeline trích xuất hóa đơn của dịch vụ OCR}
\end{figure}
...
\figland{images/img28.png}{Hình 4.17. Luồng AI hai tầng (EasyOCR + VietOCR) so với PaddleOCR hợp nhất}{Hình 4.17. Luồng AI hai tầng (EasyOCR + VietOCR) so với PaddleOCR hợp nhất}
...
\textbf{Các lớp xử lý. }(1) Đầu vào là ảnh hóa đơn (JPG/PNG). (2) Tiền xử lý (\texttt{\_to\_images}) chuẩn hóa ảnh về hệ màu RGB. (3) Tùy engine người dùng chọn (trường \texttt{ocrEngine} truyền từ backend .NET), luồng rẽ nhánh giữa ba engine tự chủ: pipeline \texttt{vietocr}, pipeline \texttt{paddleocr} và biến thể lai \texttt{paddledet\_viet} (DBNet phát hiện + VietOCR nhận dạng); cả ba đều đi tiếp qua lớp Phát hiện rồi Nhận dạng. (4) Lớp Phát hiện: pipeline vietocr dùng EasyOCR/CRAFT để tìm và cắt từng dòng, pipeline paddleocr dùng DBNet kèm bộ phân loại góc (cls). (5) Lớp Nhận dạng: VietOCR (VGG19 + Transformer) hoặc SVTR\_LCNet (PP-OCRv4, đã fine-tune) đọc chữ, tạo ra danh sách ô OCR gồm văn bản, hộp bao (toạ độ) và độ tin cậy.
...
\textbf{Đối chiếu với bảng xếp hạng AI công khai (OmniDocBench).} Ngoài bảng chấm điểm theo tiêu chí riêng ở trên, nhóm còn đối chiếu lựa chọn của mình với một \emph{bảng xếp hạng độc lập, công khai} do cộng đồng nghiên cứu duy trì để tăng tính khách quan. \textbf{OmniDocBench} (công bố tại hội nghị CVPR 2025, nhóm OpenDataLab) là bộ benchmark chuẩn cho bài toán bóc tách tài liệu (document parsing), chấm điểm end-to-end trên nhiều loại tài liệu (gồm bảng, công thức, chữ) và duy trì bảng xếp hạng cập nhật liên tục cho cả mô hình mã nguồn mở lẫn mô hình thương mại. Điều đáng chú ý cho đề tài là \textbf{PaddleOCR — chính là họ engine mà đề tài sử dụng — góp mặt và hiện dẫn đầu bảng xếp hạng này}: biến thể pipeline \texttt{PP-StructureV3} và các mô hình \texttt{PaddleOCR-VL} do cùng nhóm PaddleOCR phát triển đạt điểm tổng cao nhất, vượt cả các mô hình đa phương thức thương mại như GPT-5.2 hay Gemini~3~Pro. Trích một phần bảng xếp hạng (điểm tổng \emph{Overall}, thang 100, tại thời điểm khảo sát 07/2026):
...
3 & GLM-OCR & Mã nguồn mở & 95,22 \\ \hline
4 & \textbf{PaddleOCR-VL-1.5} \textit{(họ PaddleOCR)} & Mã nguồn mở & \textbf{94,93} \\ \hline
5 & \textbf{PaddleOCR-VL} \textit{(họ PaddleOCR)} & Mã nguồn mở & \textbf{94,18} \\ \hline
13 & Gemini 3 Pro \textit{(tham chiếu)} & VLM thương mại & 92,91 \\ \hline
\end{tabular}}\end{center}

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{images/ocr_leaderboard_omnidocbench.png}
\caption*{\small Hình 4.30b. Bảng xếp hạng AI công khai OmniDocBench (CVPR 2025) — họ engine PaddleOCR dẫn đầu bảng xếp hạng bóc tách tài liệu}
\addcontentsline{lof}{figure}{Hình 4.30b. Bảng xếp hạng AI công khai OmniDocBench — PaddleOCR dẫn đầu}
\end{figure}
\noindent\textit{\small Nguồn: OmniDocBench, OpenDataLab (CVPR 2025). Kho mã và bảng xếp hạng: \url{https://github.com/opendatalab/OmniDocBench}; trang xếp hạng trực tuyến: \url{https://opendatalab.com/OmniDocBench}; bài báo: \url{https://huggingface.co/papers/2412.07626} (truy cập 07/2026).}

\noindent Việc engine mà đề tài lựa chọn cũng chính là engine đang đứng đầu một bảng xếp hạng AI độc lập, công khai củng cố thêm cho quyết định kỹ thuật ở trên: PaddleOCR không chỉ phù hợp về chi phí và khả năng tùy biến, mà còn nằm trong nhóm dẫn đầu về độ chính xác bóc tách tài liệu theo đánh giá khách quan của cộng đồng.
...
\figland{images/img37.pdf}{Hình 4.32. Biểu đồ Use Case – Quản lý sự cố và SLA}{Hình 4.32. Biểu đồ Use Case – Quản lý sự cố và SLA}
\begin{center}{\singlespacing\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr\linewidth/3-2\tabcolsep-\arrayrulewidth\relax}|}
\hline
\textbf{Mã UC} & \textbf{Use case} & \textbf{Tác nhân} \\ \hline
...
\end{figure}
\figland{images/img39.pdf}{Hình 4.34. Sơ đồ thực thể – liên kết (ERD) phân hệ Quản lý sự cố \& SLA}{Hình 4.34. Sơ đồ thực thể – liên kết (ERD) phân hệ Quản lý sự cố \& SLA}
...
\end{tabular}}\end{center}
\noindent\textbf{Quan hệ liên kết. }Ticket là thực thể trung tâm: tham chiếu \textbf{Asset} (assetId — tài sản gặp sự cố, xem Hình 4.5), \textbf{PurchaseRequest} (purchaseRequestId — đề xuất mua sinh ra khi cần vật tư, xem Hình 4.12) và \textbf{SlaConfig} (slaConfigId — cam kết thời gian theo mức ưu tiên). Các bảng con TicketAssignment, TicketAttachment, TicketRating, TicketStatusHistory và SlaEscalationLog đều tham chiếu Ticket. Người báo/kỹ thuật viên/người leo thang là định danh cross-service sang Auth (không đặt khóa ngoại trực tiếp). Toàn bộ được thể hiện trên Hình 4.34 (nút viền đứt là bảng liên kết chi tiết ở ERD khác).
...
\endhead
\textbf{Giai đoạn 1: 25/01/2026 – 15/02/2026 — Khởi động \& khảo sát} &  &  &  &  &  &  &  \\ \hline
...
\textbf{Giai đoạn 2: 16/02/2026 – 01/03/2026 — Thiết kế CSDL \& kiến trúc chung} &  &  &  &  &  &  &  \\ \hline
Làm việc theo nhóm & \textasciitilde{}10 giờ & Cùng nhóm thiết kế CSDL dùng chung (schema base/​auth) và chuẩn khóa ngoại liên phân hệ; thống nhất quy ước đặt tên. & Sơ đồ ERD Base \& auth; chuẩn dữ liệu dùng chung cho cả 3 phân hệ. & Thiết kế CSDL quan hệ; chuẩn hóa; Git. & Đồng bộ mapping Guid liên service. & Phối hợp thiết kế tốt. & SV / GVHD \\ \hline
...
\textbf{Giai đoạn 3: 02/03/2026 – 22/03/2026 — Xây dựng module Base (làm chung cả nhóm)} &  &  &  &  &  &  &  \\ \hline
Làm việc theo nhóm & \textasciitilde{}20 giờ & Cùng nhóm xây dựng module Base dùng chung (Auth \& bảo mật, Quản lý người dùng, Thông báo, Cấu hình, Báo cáo, Lưu trữ, Audit log); cá nhân phụ trách phần Backend của Base. & Hoàn thành API Base: xác thực JWT/​refresh, middleware RBAC, CRUD tài khoản, liên kết tài khoản $\leftrightarrow$ cư dân, service thông báo, danh mục dùng chung, API dashboard. & .NET 8/​EF Core; JWT/​OAuth/​RBAC; kiến trúc module; PostgreSQL. & Chuẩn hóa hợp đồng API giữa 3 thành viên. & Phối hợp tốt; nên tài liệu hóa API sớm. & SV / GVHD \\ \hline
Làm việc theo nhóm & \textasciitilde{}8 giờ & Tích hợp Base với Frontend (Nguyễn Xuân Khải) và phần BA+Design/​Audit/​Config (Lương Thanh Tài); review chéo, thống nhất dữ liệu dùng chung. Chi tiết phân công ở bảng cuối phụ lục. & Module Base tích hợp hoàn chỉnh, làm nền cho 3 phân hệ riêng phát triển song song. & Làm việc nhóm; Git; tích hợp FE–BE; code review. & Cần thiết lập CI để giảm xung đột. & Quy trình nhóm hiệu quả. & SV / GVHD \\ \hline
\textbf{Giai đoạn 4: 23/03/2026 – 12/04/2026 — Vòng đời tài sản \& khấu hao} &  &  &  &  &  &  &  \\ \hline
...
\textbf{Giai đoạn 5: 13/04/2026 – 03/05/2026 — Bảo trì phòng ngừa (PM/WO)} &  &  &  &  &  &  &  \\ \hline
...
\textbf{Giai đoạn 6: 04/05/2026 – 24/05/2026 — Kho – Mua sắm – Nhà cung cấp} &  &  &  &  &  &  &  \\ \hline
...
\textbf{Giai đoạn 7: 25/05/2026 – 14/06/2026 — Tích hợp OCR hóa đơn} &  &  &  &  &  &  &  \\ \hline
...
\textbf{Giai đoạn 8: 15/06/2026 – 01/07/2026 — Sự cố \& SLA, hoàn thiện tính năng} &  &  &  &  &  &  &  \\ \hline
...
\textbf{Giai đoạn 9: 02/07/2026 – 20/07/2026 — Kiểm thử, viết tài liệu \& bảo vệ} &  &  &  &  &  &  &  \\ \hline
...
\textit{Ghi chú: Đồ án được giao ngày 25/01/2026 (buổi gặp GVHD tại trường); toàn bộ tính năng hoàn thành ngày 01/07/2026; giai đoạn 02–20/07/2026 dành cho kiểm thử tổng thể, viết tài liệu và chuẩn bị bảo vệ. Mỗi giai đoạn có xác nhận của SV và GVHD.}

\bigskip
\textbf{Phụ lục. Bảng phân công công việc — Module Base (CORE), làm chung cả nhóm}

Module Base (dùng chung cho cả ba phân hệ) do nhóm 03 sinh viên cùng xây dựng, phân chia theo vai trò Backend (BE) -- Frontend (FE) -- BA \& Design: \textbf{Nguyễn Xuân Thưởng} (tác giả báo cáo) phụ trách chính Backend; \textbf{Nguyễn Xuân Khải} phụ trách chính Frontend; \textbf{Lương Thanh Tài} phụ trách chính BA \& Design. Sau khi hoàn thiện Base, mỗi thành viên phát triển một phân hệ nghiệp vụ riêng chạy song song: Thưởng -- Quản lý tài sản; Khải -- Quản lý dịch vụ; Tài -- Quản lý dân cư. \textit{(Mốc thời gian trong kế hoạch chỉ mang tính tham khảo.)}

\begin{center}{\small\singlespacing\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|>{\raggedright\arraybackslash}p{\dimexpr0.19\linewidth-2\tabcolsep-\arrayrulewidth\relax}|>{\raggedright\arraybackslash}p{\dimexpr0.33\linewidth-2\tabcolsep-\arrayrulewidth\relax}|>{\centering\arraybackslash}p{\dimexpr0.16\linewidth-2\tabcolsep-\arrayrulewidth\relax}|>{\centering\arraybackslash}p{\dimexpr0.16\linewidth-2\tabcolsep-\arrayrulewidth\relax}|>{\centering\arraybackslash}p{\dimexpr0.16\linewidth-2\tabcolsep-\arrayrulewidth\relax}|}
\hline
\textbf{Nhóm chức năng} & \textbf{Chức năng} & \textbf{Backend} & \textbf{Frontend} & \textbf{BA \& Design} \\ \hline
Auth \& Security & Đăng nhập, đăng xuất, quản lý phiên \& token & Thưởng & Khải & Tài \\ \hline
 & Phân quyền theo vai trò (RBAC) & Thưởng & Thưởng & Thưởng \\ \hline
User Management & Quản lý tài khoản (CRUD người dùng) & Thưởng & Khải & Tài \\ \hline
 & Liên kết định danh (tài khoản $\leftrightarrow$ cư dân $\leftrightarrow$ căn hộ) & Thưởng & Khải & Tài \\ \hline
 & Audit log (lịch sử hoạt động toàn hệ thống) & Tài & Khải & Khải \\ \hline
Notification Engine & Gửi thông báo (push / email / SMS) & Thưởng & Khải & Khải \\ \hline
 & Template thông báo tái sử dụng & Khải & Tài & Tài \\ \hline
 & Lịch sử thông báo đã gửi & Thưởng & Khải & Khải \\ \hline
Configuration & Cấu hình hệ thống (tên khu, logo, thông số) & Tài & Khải & Khải \\ \hline
 & Danh mục dùng chung (loại phí, loại tài sản\ldots{}) & Thưởng & Khải & Tài \\ \hline
Reporting & Dashboard tổng quan toàn khu & Thưởng & Khải & Khải \\ \hline
\end{tabular}}\end{center}
\textit{Ghi chú: mỗi chức năng đều có sự tham gia của cả ba vai trò; cột ghi tên thành viên chịu trách nhiệm chính.}

\input{frag_test_appendix}

\input{frag_bug_tracker}

\bigskip
\textbf{Phụ lục. Hồ sơ minh chứng — Huấn luyện \& benchmark mô hình OCR}

Toàn bộ số liệu OCR ở mục 4.2.7 (bảng so sánh và các Hình 4.18–4.22) không phải nhập tay mà được \textbf{sinh tự động và tái lập được} bằng script đánh giá \texttt{ocr-service/training/analyze.py}, chạy trên Google Colab sau khi fine-tune và xuất mô hình. Mục này ghi lại quy trình, lệnh chạy và các tệp minh chứng để có thể kiểm chứng lại.

\textbf{a) Môi trường và dữ liệu.} Colab GPU (đánh giá độ trễ đo trên CPU cho công bằng); tập kiểm định \texttt{val.txt} gồm các dòng chữ cắt từ hóa đơn GTGT tổng hợp; bảng ký tự tiếng Việt \texttt{dict\_vi.txt}; trọng số sau fine-tune \texttt{vietocr\_invoice.pth} (VietOCR) và thư mục \texttt{rec\_vi} (PaddleOCR). Nhật ký huấn luyện PaddleOCR lưu ở \texttt{train.log}.

\textbf{b) Lệnh tái lập.} Chạy đúng một lệnh sau sẽ sinh lại bảng số liệu (\texttt{metrics\_summary.csv}) và 17 biểu đồ minh chứng:
\begin{quote}\footnotesize\ttfamily
python analyze.py --data ./dataset/rec --label ./dataset/rec/val.txt \\
\ \ --viet vietocr\_invoice.pth --rec ./inference/rec\_vi \\
\ \ --dict dict\_vi.txt --log ./output\_rec\_vi/train.log --limit 500
\end{quote}

\textbf{c) Chỉ số đo.} \emph{Seq-acc}: tỉ lệ đọc đúng \emph{toàn bộ} chuỗi (khắt khe nhất). \emph{Char-acc} $= 1 - $ CER, với CER là khoảng cách Levenshtein chia độ dài nhãn. \emph{NED}: tương đồng mức ký tự. \emph{CER<0,1}: tỉ lệ mẫu có CER dưới 0,1. \emph{ms/ảnh}: độ trễ suy luận trung bình (đo trên CPU). Mỗi chỉ số được tính lại độc lập cho từng engine trên \emph{cùng một} tập kiểm định nên so sánh là công bằng.

\textbf{d) Bảng kết quả (trích \texttt{metrics\_summary.csv}).} Số liệu gốc do \texttt{analyze.py} sinh ra chính là bảng so sánh bảy cấu hình engine (VietOCR/PaddleOCR trước–sau fine-tune, EasyOCR, Ensemble, Oracle) đã trình bày ở \textbf{mục 4.2.7.d}; không lặp lại ở đây để tránh trùng lặp.

Hai biểu đồ dưới đây được sinh trực tiếp từ bảng số liệu đó (bằng \texttt{analyze.py}), minh họa trực quan hai kết luận chính: PaddleOCR nằm ở góc "nhanh mà vẫn chính xác cao" (Hình PL.1) và bức tranh tổng hợp các chỉ số của ba engine (Hình PL.2).

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{images/bench_acc_latency.png}
\caption*{\small Hình PL.1. Đánh đổi chính xác $\leftrightarrow$ tốc độ: PaddleOCR nhanh nhất (65 ms) với độ chính xác cao (0,893); VietOCR chính xác tuyệt đối nhưng chậm hơn (317 ms)}
\addcontentsline{lof}{figure}{Hình PL.1. Đánh đổi chính xác – tốc độ giữa ba engine OCR}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth,height=0.5\textheight,keepaspectratio]{images/bench_radar.png}
\caption*{\small Hình PL.2. Radar tổng hợp ba engine trên 5 trục (Seq-acc, Char-acc, NED, Tốc độ, CER<0,1); mọi trục: càng ra ngoài càng tốt}
\addcontentsline{lof}{figure}{Hình PL.2. Radar tổng hợp ba engine OCR}
\end{figure}

\textbf{e) Chốt số từ \texttt{train.log} (PaddleOCR rec).} Ngoài bảng trên (đánh giá mức chuỗi), trong quá trình huấn luyện PaddleOCR còn tự ghi độ chính xác trên tập val: \texttt{acc = 0,9964} và \texttt{norm\_edit\_dis = 0,9992} tại epoch 41. Với tầng phát hiện (det), \texttt{hmean} tốt nhất chỉ đạt 0,2298 — cho thấy phát hiện bố cục trên dữ liệu tổng hợp còn yếu (đã nêu hướng cải thiện ở mục 4.2.7.d).

\textbf{f) Tệp minh chứng kèm theo.} Đã lưu ngay trong mã nguồn dự án tại \texttt{ocr-service/training/analysis/}: \texttt{metrics\_summary.csv} (bảng số liệu gốc) và các biểu đồ tổng hợp (Hình PL.1, PL.2 và các hình ở mục 4.2.7). Bộ đầy đủ gồm \texttt{train.log} (nhật ký huấn luyện gốc), toàn bộ 17 biểu đồ PNG do \texttt{analyze.py} sinh (đường cong huấn luyện, tác động fine-tune, so sánh engine, độ trễ, phân bố lỗi CER, cặp ký tự nhầm lẫn, giao tập đọc đúng\ldots{}), dataset hóa đơn tổng hợp và \texttt{dict\_vi.txt} — do dung lượng lớn nên lưu trong Hồ sơ minh chứng kèm theo (thư mục dự án trên Google Drive) và có thể sinh lại bằng lệnh ở mục b.
