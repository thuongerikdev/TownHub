    ...
    <main className="min-h-screen bg-background p-3 text-foreground sm:p-5">
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-7xl overflow-hidden rounded-[2rem] border border-border bg-card shadow-2xl shadow-black/10 sm:min-h-[calc(100vh-2.5rem)] lg:grid-cols-[1.08fr_.92fr]">
        <section className="relative hidden overflow-hidden bg-zinc-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
          <div className="absolute -right-24 -top-24 size-80 rounded-full bg-brand/20 blur-3xl" />
          <div className="absolute bottom-0 left-0 size-96 rounded-full bg-brand/10 blur-3xl" />
          ...
          <Link href="/" className="relative z-10 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-[.25em] text-white/50">Resident Portal</p>
            </div>
          </Link>

          <div className="relative z-10 max-w-xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-brand/25 bg-brand/10 px-3 py-1.5 text-xs text-brand">
              <Sparkles className="size-3.5" />
              Một nơi cho mọi tiện ích cư dân
            </div>
            <h1 className="text-5xl font-semibold leading-[1.08] tracking-tight">
              Kết nối cuộc sống
              <span className="block text-white/60">trong khu đô thị.</span>
            </h1>
          ...
          <div className="relative z-10 grid grid-cols-3 gap-3">
            {[["24/7", "Hỗ trợ cư dân"], ["Bảo mật", "Xác thực 2 lớp"], ["Tức thời", "Nhận thông báo"]].map(([value, label]) => (
              <div key={label} className="rounded-2xl border border-white/10 bg-white/[.04] p-4 backdrop-blur">
